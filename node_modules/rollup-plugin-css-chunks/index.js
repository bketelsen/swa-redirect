...
const cssChunks = function (options = {}) {
    ...
    return {
        ...
        load(id) {
            if (!filter(id))
                return null;
            let code = fs_1.readFileSync(id, 'utf8');
            let map = null;
            ...
            if (m !== null) {
                code = code.replace(m[0], '').trim();
                try {
                    map = fs_1.readFileSync(path_1.default.resolve(id, '..', m[1].trim()), 'utf8');
                }
                catch (err) {
                    console.warn(`Could not load css map file of ${id}.\n  ${err}`);
                }
            ...
            for (const chunk of Object.values(bundle).reverse()) {
                ...
                for (const f of css_modules) {
                    if (pluginOptions.sourcemap && emitFiles) {
                        ...
                        const decoded = sourcemap_codec_1.decode(css_data[f].map.mappings);
                        if (i === 0) {
